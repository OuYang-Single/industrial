...
import com.ijcsj.ui_library.utils.HiDisplayUtil;
import com.ijcsj.ui_library.utils.HiViewUtil;
import com.ijcsj.ui_library.widget.tab.common.IHiTabLayout;
 ...
 * 2. 中间高度超过，凸起布局
 */
public class HiTabBottomTowLayout extends FrameLayout implements IHiTabLayout<HiTabBottom, HiTabBottomInfo<?>> {
    ...
    //TabBottom高度
    private static float tabBottomHeight = 58;
    private static float tabBottomHeights = 30;
    //TabBottom的头部线条高度
    ...
    public HiTabBottomTowLayout(@NonNull Context context) {
        this(context, null);
    }

    public HiTabBottomTowLayout(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HiTabBottomTowLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
  ...
  public void setTabHeights(float tabHeight) {
        this.tabBottomHeights = tabHeight;
    }
    ...
    private void addBackground() {
        View view = LayoutInflater.from(getContext()).inflate(R.layout.hi_bottom_layout_bgs, null);

        LayoutParams params =
                new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, HiDisplayUtil.dp2px(tabBottomHeight-tabBottomHeights, getResources()));
        params.gravity = Gravity.BOTTOM;
        addView(view, params);
    ...
    public static void clipBottomPadding(ViewGroup targetView) {
        if (targetView != null) {
            targetView.setPadding(0, 0, 0, HiDisplayUtil.dp2px(targetView.getContext(),tabBottomHeight));
            targetView.setClipToPadding(false);
        }
    }
