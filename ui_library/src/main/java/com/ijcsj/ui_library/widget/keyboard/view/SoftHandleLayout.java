package com.ijcsj.ui_library.widget.keyboard.view;

import static androidx.core.content.ContextCompat.getSystemService;

import static com.ijcsj.ui_library.statusbar.StatusBarUtils.getNavigationBarHeight;
...
public class SoftHandleLayout extends SoftListenLayout {
    ...
    public SoftHandleLayout(Context context, AttributeSet attrs) {
        ...
        int screenHeight = displayMetrics.heightPixels;
         ...
         DisplayMetrics realMetrics = new DisplayMetrics(); display.getRealMetrics(realMetrics);
         int physicalHeight = realMetrics.heightPixels;
        if (screenHeight < physicalHeight-100) {
            is=false;
        } else {
            is=true;
        }
    ...
    private void setAutoViewHeight(final int height) {
        if (height == 0) {
            ...
            LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) mAutoHeightLayoutView
                    .getLayoutParams();
            if (is){
                params.height = height-(int) ScreenUtils.dp2Px(getContext(),39);;
            }else {
