...
inline fun Context.ViewPager2(
    ...
    val viewPager2 = if (style != null) ViewPager2(
        ContextThemeWrapper(this, style)
...
fun ViewGroup.Space(
    style: Int? = null,
    init: (Space.() -> Unit)? = null,
): Space {
    val space = if (style != null) Space(ContextThemeWrapper(context, style)) else Space(context)
    init?.invoke(space)
    return space.also { addView(it) }
}
